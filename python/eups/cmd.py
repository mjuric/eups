...
"""
    ...
    def addOptions(self):
        ...
        self.clo.add_option("-D", "--distrib-class", dest="distribClasses", action="append",
                            help="register this Distrib class (repeat as needed)")
        self.clo.add_option("-R", "--rebuild", dest="rebuildProductVersion", default="",
                            help="Create a new distribution given that product:version's ABI has changed")
        self.clo.add_option("--rebuildSuffix", default="",
                            help="Specify suffix to apply to new versions generated by --rebuild")
        self.clo.add_option("-S", "--server-class", dest="serverClasses", action="append",
                            help="register this DistribServer class (repeat as needed)")
        self.clo.add_option("-S", "--server-option", dest="serverOpts", action="append",
                            help="pass a customized option to all repositories " +
    ...
    def execute(self):
        ...
        #
        if self.opts.rebuildProductVersion:
            try:
                rebuildName, rebuildVersion = re.split(r"[:,]|\s+",
                                                       self.opts.rebuildProductVersion, maxsplit=1)
            except ValueError, e:
                raise RuntimeError("Please specify product:version, not \"%s\"" %
                                   self.opts.rebuildProductVersion)

            rebuildSuffix = self.opts.rebuildSuffix
            if not re.search(r"^[a-zA-Z_0-9]*$", rebuildSuffix):
                raise RuntimeError("Please restrict yourself to a-zA-Z0-9_ in rebuildSuffix; saw %s" %
                                   rebuildSuffix)                             
